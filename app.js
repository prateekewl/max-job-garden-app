...
const applicationRecordDialog = document.querySelector("#applicationRecordDialog");
...
function bindStaticEvents() {
  ...
  [jobDialog, addJobDialog, applicationRecordDialog, accountDialog].forEach((dialog) => {
...
async function loadApplications() {
  ...
  try {
    const response = await fetch("/api/applications", { cache: "no-store", credentials: "same-origin" });
    if (response.ok) {
      const payload = await response.json();
      if (Array.isArray(payload.applications) && payload.applications.length) applications = payload.applications;
    }
  } catch {
    // The local/GitHub Pages build remains useful with browser-backed records.
  }
  const merged = new Map(BUILT_IN_APPLICATIONS.map((application) => [application.id, { ...application }]));
  applications.forEach((application) => merged.set(application.id, application));
  state.applications = [...merged.values()];
  state.applications.forEach(ensureApplicationJob);
  saveLocalWorkspace();
}

function ensureApplicationJob(application) {
  let job = state.jobs.find((item) => item.id === application.jobId || (application.jobUrl && item.url === application.jobUrl));
  if (job) {
    Object.assign(job, { status: normaliseStatus(application.status), appliedDate: application.appliedAt, nextActionAt: application.nextActionAt || job.nextActionAt });
    return job;
  }
  job = normaliseJob({
    ...
    postedDate: "",
    discoveredAt: application.createdAt || application.appliedAt,
    firstSeenAt: application.createdAt || application.appliedAt,
    seenAt: application.createdAt || application.appliedAt,
    appliedDate: application.appliedAt,
    nextActionAt: application.nextActionAt,
    status: application.status || "applied",
    description: "Application submitted and preserved in the application tracker.",
  });
  state.jobs.unshift(job);
  return job;
}

async function saveApplicationRecord(form) {
  const submitButton = form.querySelector('[type="submit"]');
  submitButton.disabled = true;
  const formData = new FormData(form);
  const job = ensureTrackedJob(String(formData.get("jobId") || ""));
  if (!job) {
    submitButton.disabled = false;
    showToast("Application not found", "Reopen the role and try again.", "error");
    return;
  }
  const existing = state.applications.find((item) => item.id === form.dataset.applicationId || item.jobId === job.id);
  const appliedAt = String(formData.get("appliedAt") || localDateKey(new Date()));
  const application = {
    ...(existing || {}),
    id: existing?.id || `${slugify(`${job.company}-${job.title}`)}-${appliedAt}`,
    jobId: job.id,
    company: job.company,
    roleTitle: job.title,
    jobUrl: job.url || "",
    ...
    appliedAt,
    status: normaliseStatus(formData.get("status") || "applied"),
    message: String(formData.get("message") || "").trim(),
    contactName: String(formData.get("contactName") || "").trim(),
    contactEmail: String(formData.get("contactEmail") || "").trim(),
    nextActionAt: existing?.nextActionAt || addDaysKey(appliedAt, state.preferences.followUpDays),
    cvFilename: existing?.cvFilename || "",
    cvKey: existing?.cvKey || "",
    cvDataUrl: existing?.cvDataUrl || "",
    coverLetterFilename: existing?.coverLetterFilename || "",
    coverLetterKey: existing?.coverLetterKey || "",
    coverLetterDataUrl: existing?.coverLetterDataUrl || "",
  ...
  try {
    ...
    upload.set("application", JSON.stringify(application));
    if (file instanceof File && file.size) upload.set("cv", file, file.name);
    if (coverLetter instanceof File && coverLetter.size) upload.set("coverLetter", coverLetter, coverLetter.name);
    const response = await fetch("/api/applications", { method: "POST", body: upload, credentials: "same-origin" });
    if (!response.ok) throw new Error((await response.json().catch(() => ({}))).error || "Private storage is unavailable.");
    const payload = await response.json();
    Object.assign(application, payload.application);
  } catch {
    if (file instanceof File && file.size) {
      application.cvFilename = file.name;
      application.cvContentType = file.type || "application/pdf";
      application.cvDataUrl = await fileToDataUrl(file);
    }
    if (coverLetter instanceof File && coverLetter.size) {
      application.coverLetterFilename = coverLetter.name;
      application.coverLetterContentType = coverLetter.type || "application/pdf";
      application.coverLetterDataUrl = await fileToDataUrl(coverLetter);
    }
  }
  ...
  state.view = "pipeline";
  render();
  showToast("Application recorded", `${application.company} · application files and message saved together`, "success");
  submitButton.disabled = false;
}

async function updateApplicationStatus(applicationId, value) {
  const application = state.applications.find((item) => item.id === applicationId);
  if (!application) return;
  application.status = normaliseStatus(value);
  application.updatedAt = new Date().toISOString();
  const job = ensureApplicationJob(application);
  updateJob(job.id, { status: application.status }, "Application status updated", true);
  saveLocalWorkspace();
  try {
    await fetch(`/api/applications/${encodeURIComponent(application.id)}`, {
      method: "PATCH", credentials: "same-origin", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status: application.status, nextActionAt: application.nextActionAt || "" }),
    });
  } catch { /* Browser-backed record remains current. */ }
  openApplicationPack(application.id);
  showToast("Status updated", `${application.company} · ${titleCase(application.status.replace(/_/g, " "))}`, "success");
}
...
function renderPipeline(jobs, stats) {
  ...
  return `
    <div class="page-stack">
      ...
      <section class="section application-tracker-section">
        <div class="section-heading"><div><p class="eyebrow">Submitted applications</p><h2>What Max actually sent</h2><p>Every application keeps its exact CV, message, contact, dates and next step together.</p></div><strong class="application-total">${state.applications.length} recorded</strong></div>
        ${state.applications.length ? `<div class="application-records">${[...state.applications].sort((a, b) => String(b.appliedAt).localeCompare(String(a.appliedAt))).map(renderApplicationRecord).join("")}</div>` : renderEmpty("file", "No submitted applications yet", "When Max marks a role as applied, Job Garden will save the exact message and CV used.")}
      </section>
    </div>`;
}

function renderApplicationRecord(application) {
  const cvReady = Boolean(application.cvKey || application.cvDataUrl);
  const coverLetterReady = Boolean(application.coverLetterKey || application.coverLetterDataUrl);
  return `<article class="application-record">
    <div class="application-record-main">
      <div class="meta-row">${statusPill(application.status)}<span class="meta-pill">Applied ${escapeHtml(formatDate(application.appliedAt, { year: false }) || application.appliedAt)}</span></div>
      <h3>${escapeHtml(application.roleTitle)}</h3><p>${escapeHtml(application.company)} · ${escapeHtml(application.location || "Location not recorded")}</p>
      <div class="application-evidence"><span class="${application.message ? "ready" : ""}"><svg aria-hidden="true"><use href="#icon-mail"></use></svg>${application.message ? "Message saved" : "Message missing"}</span><span class="${cvReady ? "ready" : ""}"><svg aria-hidden="true"><use href="#icon-file"></use></svg>${cvReady ? "CV attached" : "CV needs attachment"}</span><span class="${coverLetterReady ? "ready" : ""}"><svg aria-hidden="true"><use href="#icon-file"></use></svg>${coverLetterReady ? "Cover letter attached" : "No cover letter"}</span>${application.nextActionAt ? `<span><svg aria-hidden="true"><use href="#icon-clock"></use></svg>Follow up ${escapeHtml(formatDate(application.nextActionAt, { year: false }) || application.nextActionAt)}</span>` : ""}</div>
    </div>
    <div class="application-record-actions"><button class="button primary small" type="button" data-open-application="${escapeHtml(application.id)}">View application pack</button>${application.jobUrl ? `<a class="button quiet small" href="${escapeHtml(application.jobUrl)}" target="_blank" rel="noreferrer">Advert</a>` : ""}</div>
  </article>`;
}
...
function openJob(jobId) {
  jobDialog.classList.remove("application-pack-dialog");
...
function openApplicationPack(applicationId) {
  const application = state.applications.find((item) => item.id === applicationId);
  if (!application) return;
  const cvHref = application.cvKey ? `/api/applications/${encodeURIComponent(application.id)}/cv` : application.cvDataUrl || "";
  const cvPreviewHref = application.cvKey ? `${cvHref}?preview=1#toolbar=0&navpanes=0&view=FitH` : cvHref;
  const canPreviewCv = Boolean(cvPreviewHref) && (application.cvContentType || "application/pdf").includes("pdf");
  ...
  jobDialogContent.innerHTML = `<div class="job-dialog-shell application-pack-shell">
    ...
    <div class="job-detail application-pack-detail">
      <section class="application-pack-hero"><div><p class="eyebrow">Application pack</p><h2 id="jobDialogTitle">${escapeHtml(application.roleTitle)}</h2><p>${escapeHtml(application.company)} · submitted ${escapeHtml(formatDate(application.appliedAt) || application.appliedAt)}</p></div>${statusPill(application.status)}</section>
...
function openApplicationRecordDialog(job, existing = null) {
  const form = document.querySelector("#applicationRecordForm");
  if (!form || !job) return;
  form.reset();
  form.elements.jobId.value = job.id;
  form.elements.appliedAt.value = existing?.appliedAt || localDateKey(new Date());
  form.elements.status.value = existing?.status || "applied";
  form.elements.contactName.value = existing?.contactName || "";
  form.elements.contactEmail.value = existing?.contactEmail || "";
  form.elements.message.value = existing?.message || buildMessage("recruiter", job, state.profile);
  form.dataset.applicationId = existing?.id || "";
  document.querySelector("#applicationRecordTitle").textContent = existing ? `Edit ${job.company} application` : `Record ${job.company} application`;
  applicationRecordDialog.showModal();
  updateDialogBodyState();
}
...
function handleDocumentClick(event) {
  ...
  const applicationButton = event.target.closest("[data-open-application]");
  if (applicationButton) {
    openApplicationPack(applicationButton.dataset.openApplication);
    return;
  }
  const applicationAction = event.target.closest("[data-application-action]");
  if (applicationAction) {
    const application = state.applications.find((item) => item.id === applicationAction.dataset.applicationId);
    if (!application) return;
    if (applicationAction.dataset.applicationAction === "copy-message") {
      copyText(application.message).then(() => showToast("Message copied", `${application.company} · ${application.roleTitle}`, "success"));
    } else if (applicationAction.dataset.applicationAction === "edit") {
      const job = ensureApplicationJob(application);
      jobDialog.close();
      openApplicationRecordDialog(job, application);
    }
    return;
  }
...
function handleDocumentChange(event) {
  ...
  if (target.matches("[data-application-status]")) {
    updateApplicationStatus(target.dataset.applicationStatus, target.value);
    return;
  }
...
function handleDocumentSubmit(event) {
  ...
  if (event.target.id === "applicationRecordForm") {
    event.preventDefault();
    saveApplicationRecord(event.target);
    return;
  }
...
async function handleJobAction(action, jobId) {
  ...
  if (action === "save") {
    ...
    openApplicationRecordDialog(job, state.applications.find((item) => item.jobId === job.id));
    return;
...
function saveLocalWorkspace() {
  ...
  const payload = {
    ...
    applications: state.applications.slice(0, 250),
    scout: state.scout,
...
function updateDialogBodyState() {
  document.body.classList.toggle("dialog-open", [jobDialog, addJobDialog, applicationRecordDialog, accountDialog].some((dialog) => dialog?.open));
}
